...
- **Phase/task:** Task 8 worktree preflight
- **Symptom:** `git check-ignore -v .worktrees` was run from inside the existing worktree, where that relative target does not exist, and returned exit 1 after confirming the worktree identity.
- **Impact:** No project file changed and no test or provider command ran from the affected check.
- **Confirmed cause:** The ignore target is repository-root-relative, but the check used the nested worktree as its working directory.
- **Correction:** Rerun `git check-ignore -v .worktrees` from the primary repository root.
- **Prevention:** Resolve the repository that owns the project-local worktree container before evaluating its ignore rule.
- **Related verification:** The root-level check matched `.gitignore:1:.worktrees/`; the supplied worktree remained on `codex/deterministic-router-v1` at the requested HEAD before edits.
